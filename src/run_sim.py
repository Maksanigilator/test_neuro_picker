...
import numpy as np
import time
from src.environment import BinPickingEnv
...
model.load_state_dict(torch.load("picker_net_interrupted_large.pth", map_location=device))
...
while box_ids:
    ...
    for uid in unique_ids:
        ...
        input_crop = np.stack([depth_crop, mask_crop], axis=0)
    ...
    inputs_tensor = torch.from_numpy(np.stack(inputs)).to(device)
    zs_tensor = torch.tensor(zs).unsqueeze(1).to(device)
    zs_norm = (zs_tensor - 0.3) / (2.0 - 0.3)  # Normalize as in train

    with torch.no_grad():
        logits = model(inputs_tensor, zs_norm)
